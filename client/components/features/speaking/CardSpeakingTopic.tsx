...
export default function CardSpeakingTopic({
  _id,
  title,
  description,
  level,
  ...
  completed,
  total,
}: CardSpeakingTopicProps) {
  const progress = (completed / total) * 100;

  const getCardStyle = () => {
    if (isCompleted) return "bg-gradient-to-br from-orange-50 border-2 via-teal-50 to-cyan-50 border-orange-300 shadow-xl hover:shadow-2xl";
    if (isUnlocked && !isCompleted) return "bg-white border-2 border-orange-300 shadow-xl hover:shadow-2xl";
  ...
  const getIconColor = () => {
    ...
    if (isUnlocked && !isCompleted) return "bg-orange-100 text-orange-600";
    return "bg-gray-100 text-gray-400";
  };

  const getTitleColor = () => {
    if (isCompleted) return "text-gray-800 group-hover:text-emerald-600";
    if (isUnlocked && !isCompleted) return "text-gray-800 group-hover:text-orange-600";
  ...
  return (
    <Popover>
      <PopoverTrigger className='w-full text-center group'>
        <Card className={`relative flex flex-col h-full justify-between transition-all duration-500 ease-out p-6 ${getCardStyle()} hover:scale-105 overflow-hidden`}>
          ...
          <CardFooter className="p-0 space-y-3 flex flex-col">
            ...
            <Progress value={progress} className=" bg-orange-100! [&>div]:bg-orange-500!" />
      ...
      {isUnlocked ? (
        <EnhancedPopover
          href={`/skills/speaking/lesson/${_id}`}
          skill="speaking"
        />
      ) : (
